...
func CreateBid(bid *models.Bid) error {
	query := `INSERT INTO bids (id, name, description, status, tender_id, author_id, version, created_at) 
	...
	_, err := GetDB().Exec(query, bid.ID, bid.Name, bid.Description, bid.Status, bid.TenderID, bid.AuthorID, bid.Version, bid.CreatedAt)
	if err != nil {
